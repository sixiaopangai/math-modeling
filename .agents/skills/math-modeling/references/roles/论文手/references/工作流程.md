...
3. 在复制件中填充正文、公式、真实图表和已核验的 BibTeX 条目，不修改原始模板或 `SKILL_ROOT`。每个子问题至少安排一幅标签以 `fig:qN-` 开头的正式结果图。
4. 按官方模板要求选择 XeLaTeX、LuaLaTeX 或 pdfLaTeX，优先由 `latexmk` 编译；不启用 shell escape，不自动安装宏包，不在失败后擅自换引擎。
5. 消除编译错误、未解析交叉引用和未解析文献；每个图表都有题注、真实内容与 `label`，并由图表环境外的正文 `ref`。同时检查源码—PDF 哈希、实际页数、空白页、页面尺寸、字体嵌入和图片 DPI。
...
python ../../../../tools/latex/scripts/latex_paper.py validate "<PROJECT_ROOT>/完整论文-LaTeX/main.tex" --pdf "<PROJECT_ROOT>/完整论文.pdf" --contest cumcm --quality-checks --questions q1 q2 q3 --min-image-dpi 300 --max-pages <当届官方上限>
```

构建与转换警告默认阻断发布；只有逐项核对后才能用精确允许正则和具体覆盖理由继续。降低默认质量目标也必须记录官方条款或用户要求。自动校验通过后，再打开实际编译 PDF，抽检摘要页、分页、公式、图表、页眉页脚、字体和参考文献。
