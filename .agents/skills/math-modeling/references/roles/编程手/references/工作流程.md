...
2. 从题目分析报告列出全部子问题并规范为 `q1…qN`；为每张候选图明确对应子问题、核心结论、所属类别、证据映射、统计口径和最终尺寸；需要选图时加载 `图表选择与避坑.md`。
3. 从 `ROLE_ROOT/scripts/` 复制 Python 或 MATLAB 出版样式工具到 `PROJECT_ROOT/utils/`，再由项目代码调用。
4. 按 `raw_q1_*`、`process_q1_*`、`result_q1_*` 等命名生成三类图，每类至少 3 张逻辑候选图、合计至少 9 张，且每个子问题在三类中各至少 1 张；每张同时导出 SVG 与 300 DPI PNG，不设上限。
5. 运行 `python "<SKILL_ROOT>/references/roles/编程手/scripts/figure_audit.py" "<PROJECT_ROOT>/figures" --questions q1 q2 ... qN --strict`，并实际打开 PNG 进行视觉检查；发现问题回到源代码修正并重新运行。
